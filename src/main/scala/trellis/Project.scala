...
object Project:
  ...
  object CodeView extends Renderer:
    def render(graph: Graph, selection: Option[Selection] = None): Rendered =
      val entitiesByNode = graph.entities.toVector.groupMap(_._2)(_._1)
      val rows = graph.nodes.toVector.sortBy(_._1.value).map { case (id, node) =>
        val names = entitiesByNode.getOrElse(id, Vector.empty).map(_.value).sorted
      ...
      val marks = graph.entities.toVector.map { case (e, _) => SemanticMark(e.value, Selection.Entity(e)) }
      Rendered(rows.mkString("\n"), marks)

  object Svg extends Renderer:
    def render(graph: Graph, selection: Option[Selection] = None): Rendered =
      val ordered = graph.nodes.toVector.sortBy(_._1.value)
      val pos = ordered.zipWithIndex.map { case ((id, _), i) => id -> (40, 40 + i * 90) }.toMap
      ...
      val nodes = ordered.map { case (id, node) =>
        val (x, y) = pos(id)
        val label = xmlEscape(graph.entities.collectFirst { case (e, nid) if nid == id => e.value }.getOrElse(node.kind))
      ...
      val svg = s"""<svg xmlns="http://www.w3.org/2000/svg" width="640" height="$height" viewBox="0 0 640 $height">
                   |$body
                   |</svg>""".stripMargin
      val marks = ordered.map { case (id, _) => SemanticMark("node-" + id.value, Selection.Node(id)) } ++
        graph.edges.keys.toVector.map(id => SemanticMark("edge-" + id.value, Selection.Edge(id)))
      Rendered(svg, marks)
  ...
  object Typst extends Renderer:
    def render(graph: Graph, selection: Option[Selection] = None): Rendered =
      ...
      val marks = graph.entities.keys.toVector.map(e => SemanticMark(e.value, Selection.Entity(e)))
      Rendered(doc, marks)
